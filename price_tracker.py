"""
On the Spot Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from On the Spot.

Docs  : https://docs.bright.cn/scraping-automation/web-scraper-api/overview
Info  : https://bright.cn/products/insights/price-tracker/on-the-spot
"""
...
# Set your Web Scraper ID after building your On the Spot scraper:
# https://bright.cn/products/web-scraper/on-the-spot
DATASET_ID = os.getenv("BRIGHTDATA_DATASET_ID", "")
if not DATASET_ID:
    raise EnvironmentError(
        "BRIGHTDATA_DATASET_ID is not set.\n"
        "1. Build your On the Spot scraper at: https://bright.cn/products/web-scraper/on-the-spot\n"
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of On the Spot product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover On the Spot products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a On the Spot category page.

    Example:
        results = discover_by_category("https://onthespot.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://www.onthespot.com/product/sample-item-123456",
        # Add more On the Spot product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "on-the-spot_prices.json"
    ...
    # cat_results = discover_by_category("https://onthespot.com/category/sample", limit=50)
